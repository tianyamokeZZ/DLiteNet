...
import time
from models.DSNet.model_utils import BasicBlock, Bottleneck, segmenthead, AFF, ASPP, CARAFE, segmentheadCARAFE, iAFF, segmenthead_drop, Muti_AFF, segmenthead_c, DUC, SPASPP, MFACB
import logging
import os
...
if __name__ == "__main__":
    model = DSNet(m=2, n=2, num_classes=1, planes=32, name='s128', augment=False).cuda()
    # 模型训练模式
    model.train()
    # 模拟输入图像
    image = torch.randn(4, 3, 512, 512).cuda()  # (batch_size, channels, height, width)
    # 输出模型结构和输入输出形状
    # print(model)
